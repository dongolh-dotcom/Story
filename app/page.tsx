...
export default function Home() {
  ...
  const login = (user: User) => {
    ...
    showToast(`Welcome back, ${user.name}! 鉁╜);
  };
  ...
  const handleNewStory = (title: string, opening: string) => {
    ...
    setShowNewModal(false);
    showToast("馃帀 Story published successfully!");
  };
  ...
  const handleEditStory = (title: string, opening: string, id?: string) => {
    ...
    setEditingStory(null);
    setShowNewModal(false);
    showToast("鉁� Story updated successfully!");
  };
  ...
  const confirmDelete = () => {
    if (storyToDelete) {
      ...
      showToast("馃棏锔� Story deleted successfully");
      setStoryToDelete(null);
    }
  };
  ...
  const handleAddContinuation = (text: string) => {
    ...
    showToast("馃尡 Continuation added!", 'continue');
  };
